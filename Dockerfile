...
RUN apt-get update -y && apt-get install -y curl build-essential git \
    && apt remove -y nodejs nodejs-doc && apt-get clean && rm -f /var/lib/apt/lists/*_*

RUN curl -sL https://deb.nodesource.com/setup_18.x  | bash -
RUN apt install -yq nodejs
RUN npm install -g yarn && npm cache clean --force && npm install -g typescript
...
# compile assets - remove node_modules to ensure clean install for target architecture
# yarn.lock provides version consistency; skip --frozen-lockfile to allow
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/wanderer_ops ./
