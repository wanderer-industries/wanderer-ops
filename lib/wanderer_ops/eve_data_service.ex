defmodule WandererOps.EveDataService do
  @moduledoc """
  ...
  alias WandererOps.Utils.{JSONUtil, CSVUtil}
  ...
  def download_file(file_name) do
    ...
    download_path = Path.join([:code.priv_dir(:wanderer_ops), "repo", "data", file_name])
  ...
  def load_wormhole_types() do
    JSONUtil.read_json!("#{:code.priv_dir(:wanderer_ops)}/repo/data/wormholes.json")
    |> Enum.map(fn row ->
      %{
        id: row["typeID"],
        name: row["name"],
  ...
  def load_wormhole_classes() do
    JSONUtil.read_json!("#{:code.priv_dir(:wanderer_ops)}/repo/data/wormholeClasses.json")
    |> Enum.map(fn row ->
      %{
        id: row["id"],
  ...
  def load_wormhole_systems() do
    JSONUtil.read_json!("#{:code.priv_dir(:wanderer_ops)}/repo/data/wormholeSystems.json")
    |> Enum.map(fn row ->
      %{
  ...
  def load_effects() do
    JSONUtil.read_json!("#{:code.priv_dir(:wanderer_ops)}/repo/data/effects.json")
    |> Enum.map(fn row ->
      %{
  ...
  def load_triglavian_systems() do
    JSONUtil.read_json!("#{:code.priv_dir(:wanderer_ops)}/repo/data/triglavianSystems.json")
    |> Enum.map(fn row ->
      %{
        solar_system_id: row["solarSystemID"],
  ...
  def load_wormhole_classes_info() do
    {:ok, data} =
      JSONUtil.read_json("#{:code.priv_dir(:wanderer_ops)}/repo/data/wormholeClassesInfo.json")
  ...
  def load_shattered_constellations() do
    {:ok, data} =
      JSONUtil.read_json(
        "#{:code.priv_dir(:wanderer_ops)}/repo/data/shatteredConstellations.json"
      )
  ...
  def load_map_constellations() do
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/mapConstellations.csv",
      fn row ->
        %{
  ...
  def load_map_regions() do
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/mapRegions.csv",
      fn row ->
        %{
  ...
  def load_map_location_wormhole_classes() do
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/mapLocationWormholeClasses.csv",
      fn row ->
        %{
  ...
  def load_inv_groups() do
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/invGroups.csv",
      fn row ->
        %{
  ...
  def get_db_data() do
    ...
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/mapSolarSystems.csv",
      fn row ->
  ...
  def get_ship_types_data() do
    ...
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/invTypes.csv",
      fn row ->
        group_id = row["groupID"] |> Integer.parse() |> elem(0)
  ...
  def get_solar_system_jumps_data() do
    CSVUtil.csv_row_to_table_record(
      "#{:code.priv_dir(:wanderer_ops)}/repo/data/mapSolarSystemJumps.csv",
      fn row ->
        %{
